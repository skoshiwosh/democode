...
"""
    ...
    Update: January, 2026 - converted os.path to pathlib Path
"""
...
def findtabs(topdir,skipversions = False):
    ...
    for root,dirs,files in Path(topdir).walk():
        if skipversions and root.parts[-1] == 'versions':
        ...
        pyfiles = [each for each in files if Path(each).suffix == '.py']
        if pyfiles:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        patharg = Path(sys.argv[1])
        topdir = patharg.resolve()
    else:
